...
void reset_ball(Side side) {
  ...
  float angle = (rand() / (RAND_MAX/0.5)) + 0.5;
  float flip = (rand() / (RAND_MAX/1.0));
  app_log(APP_LOG_LEVEL_DEBUG, "none", 0, "angle: %d", (int)(angle*100));
  app_log(APP_LOG_LEVEL_DEBUG, "none", 0, "flip: %d", (int)(flip*100));
  if (flip > 0.5) angle = angle * -1.0;
  if (side == pl1.side) {
    ball.vetctor = (MovementVector){1, angle};
  } else {
    ball.vetctor = (MovementVector){-1, angle};
  }
}
...
void draw_ball(GContext *ctx, Ball b) {
  graphics_fill_rect(ctx, GRect(b.position.x, b.position.y, b.size.w, b.size.h), 4, GCornersAll);
}
